const { WholesaleBullen } = require('../models');
...
exports.getBullenNames = async (req, res) => {
  try {
    const bullenNames = await WholesaleBullen.findAll({
...
exports.getBullenLedger = async (req, res) => {
  try {
    ...
    const entries = await WholesaleBullen.findAll({
      where: { bullenName },
...
exports.createBullen = async (req, res) => {
  try {
    ...
    const bullen = await WholesaleBullen.create({
      date,
...
exports.deleteBullen = async (req, res) => {
  try {
    ...
    await WholesaleBullen.destroy({ where: { id } });
...
exports.exportToExcel = async (req, res) => {
  try {
    ...
    const worksheet = workbook.addWorksheet('Wholesale Bullen');
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename=wholesale_bullen_${new Date().toISOString().split('T')[0]}.xlsx`
    );
